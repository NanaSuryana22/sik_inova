...
<div class="row clearfix">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>Tambah Data Kota di Wilayah {{ $wilayah->name }}</h2>
                </div>
                <div class="body">
                    <form id="form_validation" action="{{ route('kota.store') }}" method="POST">
                        {{ csrf_field() }}
                        <div class="form-group form-float">
                            <div class="form-line">
                                <input type="hidden" id="wilayah_id" name="wilayah_id" value="{{ $wilayah->id }}">
                                ...
                                <input type="text" class="form-control @error('wilayah_id') is-invalid @enderror"
                                    autocomplete="off" required value="{{ $wilayah->name }}" disabled>
                            </div>
                        ...
                        <div class="form-group form-float">
                            <div class="form-line">
                                ...
                                <textarea name="description" cols="30" rows="5" class="form-control no-resize @error('description') is-invalid @enderror" required>{{ old('description') }}</textarea>
                            </div>
